...
authn=stauth.Authenticate(credentials,"olymp_dashboard","abcdef")
name,authn_stat,username=authn.login("Login","main")
if authn_stat==False:
...
if authn_stat==True:
    ...
    authn.logout("Logout","sidebar")
    st.sidebar.image('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSQmURsuT8ajkOZChesWwigA2t4_WsdpZqbSA&usqp=CAU')
    user_menu = st.sidebar.radio(
        'Select any one search type',
    ...
    if user_menu == 'Overall Analysis':
        ...
        st.title("No of Participating Nations over the Years")
        st.plotly_chart(fig)
        ...
        for trace in fig.data:
            trace.mode = 'lines+markers+text'
        st.title("Total No of Events over the Years")
        st.plotly_chart(fig)
        ...
        st.title("Most successful Athletes🏃‍♂️")
    ...
    if user_menu == 'Country-wise Analysis':
        ...
        for trace in fig.data:
            trace.mode = 'lines+markers+text'
        st.title(selected_country + " Medal streak over the Years")
        st.plotly_chart(fig)
