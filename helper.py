...
def data_over_time(df, col):
    ...
    nations_over_time=nations_over_time[nations_over_time['Year']<2020]
    nations_over_time=nations_over_time.sort_values('Year',ascending=False)
...
def weight_v_height(df,sport):
    athlete_df = df.drop_duplicates(subset=['Name', 'region', 'Height', 'Weight'])
...
def men_vs_women(df):
    ...
    men = men[men['Year'] != men['Year'].max()]  # Exclude the last year
    men = men.groupby('Year').count()['Name'].reset_index()
